...
<head>
    ...
    <title>Kasir {{ session('type') === 'grosir' ? 'Grosir' : 'Ecer' }}</title>
...
<body class="bg-gray-100">
    <!-- Header Section -->
    <header class="{{ session('type') === 'grosir' ? 'bg-green-600' : 'bg-blue-600' }}">
        <div class="container mx-auto px-6 py-4">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-4">
                    <h1 class="text-xl font-bold text-white">
                        Kasir {{ session('type') === 'grosir' ? 'Grosir' : 'Ecer' }}
                    </h1>
                    @if(session('type') === 'grosir' && isset($customer))
                ...
                <div class="flex items-center space-x-6">
                    <div class="text-right">
                        ...
                        <p class="font-medium text-white">{{ session('username') }}</p>
                    </div>
                    <div class="text-right">
        ...
        <div class="p-6">
            ...
            <div class="mb-6 bg-white rounded-lg shadow-md p-4">
                <div class="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div class="flex items-center space-x-4">
                        <span class="bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Shift: {{ session('shift') ?? 'Default' }}</span>
                        <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded-full">Kas: Rp {{ number_format(session('kas') ?? 0, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
                    ...
                    <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
                        ...
                        <div class="border-t pt-4">
                            <div class="flex justify-between items-center mb-3">
                                ...
                                <span class="text-sm text-gray-500">{{ session('type') === 'grosir' ? 'Grosir' : 'Ecer' }}</span>
                            </div>
                ...
                <div class="bg-white rounded-lg shadow-md p-6 space-y-6">
                    ...
                    <div class="pt-4">
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-center">
                                <span class="text-gray-600">PPN ({{$pajak}}%)</span>
    ...
    <!-- Modal Components -->
    @if(session('type') === 'grosir')
        @include('kasir.components.payment-modal-grosir')
    @else
        @include('kasir.components.payment-modal-ecer')
    @endif
    @include('kasir.components.hold-transaction-modal')
    @include('kasir.components.held-transactions-modal')
    @include('kasir.components.guide-modal')
